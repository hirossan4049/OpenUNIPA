...
export default class AccountController extends BaseController {
  async login() {
    ...
    const { state, element } = await this.session.request!.fetch(this.session.univ!.loginPath, undefined, "GET", "<form", "</form>")
    ...
    const { state: state2, element: element2 } = await this.session.request!.fetch(loginUrl ?? this.session.univ!.loginPath, params, "POST")
